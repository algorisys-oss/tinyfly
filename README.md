...
- **Symbols & Library** - Bundle elements into a reusable **symbol** (Convert to Symbol), place instances across scenes, and **edit in place** (double-click an instance) so every instance updates — all JSON-serialized (nested playback & symbol-swap coming)
