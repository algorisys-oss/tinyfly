...
- **Symbols & Library** - Bundle elements into a reusable **symbol** (Convert to Symbol), place instances across scenes, **edit in place** (double-click an instance), and give a symbol its own timeline so every instance plays that **nested animation** — all JSON-serialized (symbol-swap/lip-sync coming)
