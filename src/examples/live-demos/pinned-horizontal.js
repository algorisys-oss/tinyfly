const panels = ['Discover', 'Design', 'Animate', 'Ship']
  .map((word, i) => `<div class="ph-panel" style="--hue:${210 + i * 40}"><span>0${i + 1}</span>${word}</div>`)
  .join('')
...
export const pinnedHorizontal = {
  ...
  description: 'Scroll inside the card: the section pins while vertical scrolling slides its panels sideways — scrollTrigger with pin and scrub.',
  tags: ['scroll', 'scrollTrigger', 'pin', 'scrub'],
