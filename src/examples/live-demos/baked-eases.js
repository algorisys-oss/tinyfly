export const html = `<style>
  ...
  .be-label { width: 118px; flex-shrink: 0; color: #888; font: 11px ui-monospace, monospace; text-align: right; }
...
<div class="be-wrap">
  ...
  <div class="be-row"><span class="be-label">elastic.out(1, 0.35)</span><div class="be-ball" id="be-elastic"></div></div>
...
export function run(live) {
  // #region code
  // Elastic, bounce and steps have no cubic-bezier form. They compile to
  // parametric eases — { type: 'elastic', mode: 'out', … } — one small keyframe
  // in the JSON, evaluated exactly when played.
  const tl = live.timeline({ repeat: -1, repeatDelay: 0.6 })

  tl.fromTo('#be-power', { x: 0 }, { x: 130, duration: 1.2, ease: 'power2.out' })
  tl.fromTo('#be-elastic', { x: 0 }, { x: 130, duration: 1.2, ease: 'elastic.out(1, 0.35)' }, '<')
...
export const bakedEases = {
  ...
  description: 'GSAP eases with no bezier equivalent, as native parametric eases — compared against power2.out.',
  tags: ['easing', 'elastic.out', 'bounce.out', 'steps', 'parametric eases'],
  html,
  run,
}
