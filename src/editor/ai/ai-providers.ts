 ...
 * All calls use fetch() directly — no library dependencies (per tinyfly's
 * "every dependency must justify its existence" rule).
 */
...
function formatHttpError(provider: string, status: number, body: string): string {
  // Try to extract a human message from the JSON error body.
  let detail = ''
  try {
  ...
  switch (status) {
    case 401:
      return `Invalid API key for ${provider}. Check your key in AI Settings.`
    case 403:
      return `Access denied by ${provider}. ${detail || 'Check API key permissions.'}`
    case 429:
      return `Rate limited by ${provider}. Please wait a moment and try again.`
    case 500:
    case 502:
    case 503:
      return `${provider} server error (${status}). Please try again later.`
    default:
      return `${provider} error ${status}: ${detail || 'Unknown error'}`
  }
}

function formatNetworkError(provider: string, err: unknown): string {
  ...
  return `Network error calling ${provider}: ${e?.message || 'Unknown error'}`
}
